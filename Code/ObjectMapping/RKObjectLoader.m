...
- (void)didFailLoadWithError:(NSError *)error {
	...
	if (_cachePolicy & RKRequestCachePolicyLoadOnError &&
        ...
        if (!self.response || [self.response isFailure]) {
            RKResponse *rsp = self.response ? self.response : [NSNull null];
            NSDictionary* userInfo = [NSDictionary dictionaryWithObjectsAndKeys:
                                     error, RKRequestDidFailWithErrorNotificationUserInfoErrorKey,
                                     rsp, RKRequestDidFailWithErrorNotificationUserInfoResponseKey, nil];
